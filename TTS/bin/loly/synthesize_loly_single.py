...
def loly_tts(sentence, model_path=None, config_path=None, speakers_file_path=None, use_cuda=False, out_path='loly-audios/',
             vocoder_path=None, vocoder_config_path=None, encoder_path=None, encoder_config_path=None,):
    ...
    new_dict = {'archivo': save_path, 'texto': texto, 'duracion': str(duration)}
...
def main():
    ...
    parser.add_argument(
        "--dialog",
        type=str,
        default="Hola soy loli y no puedo hablar bien",
        help="dialogo que dira loli",
    )

    parser.add_argument(
        "--json_path",
        type=str,
        default="TTS/bin/loly",
        help="Ruta donde estan los jsons generar.json y audios.json",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="1",
        help="ID del evento que usara este dialogo",
    )
    ...
    # load model manager
    path = "/Users/adriantomala/Desktop/TTS-main/TTS/.models.json"
    ...
    if args.dialog is not None:
        ...
        dialog = 'HOLA SOY LOLI'

    nueva_data = {}
    texto = dialog
    wav = synthesizer.tts(texto, args.speaker_idx, args.speaker_wav)
    save_path = os.path.join(args.out_path,str(event_id)+'.wav')
    synthesizer.save_wav(wav, save_path)
    fname = save_path
    with contextlib.closing(wave.open(fname, 'r')) as f:
        frames = f.getnframes()
        ...
        duration = frames / float(rate)
    new_dict = {'archivo': save_path, 'texto': texto, 'duracion': str(duration)}
    nueva_data[str(event_id)] = new_dict
    print(str(duration))
