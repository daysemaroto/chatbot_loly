...
def main():
    ...
    f = open(os.path.join(json_path, 'generar.json'))
    data = json.load(f)
    nueva_data = {}
    data_json = data['loly']
    for key in data['loly']:
        texto = data_json[key]['texto']
        wav = synthesizer.tts(texto, args.speaker_idx, args.speaker_wav)
        save_path = os.path.join(args.out_path,str(key)+'.wav')
        ...
        nueva_data[str(key)] = new_dict
    with open('TTS/bin/loly/audios.json', 'w') as fp:
        json.dump(nueva_data, fp)
